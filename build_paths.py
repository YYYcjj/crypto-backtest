...
DAYS = 60  # 拉取最近 60 天 15m K线（覆盖近 2 个月）
...
def generate_full_path(trade, candles, entry_idx):
    ...
    for j in range(start_idx, len(candles)):
        ...
        all_raw.append(c['c'])
        all_times.append(datetime.fromtimestamp(ts/1000, tz=timezone.utc).astimezone(CST).strftime('%m-%d %H:%M'))
...
def generate_minimal_path(trade):
    ...
    steps = 20
    entry_dt = datetime.strptime(trade['entry_time'][:16], '%Y-%m-%d %H:%M').replace(tzinfo=CST)
    exit_dt = datetime.strptime(trade['exit_time'][:16], '%Y-%m-%d %H:%M').replace(tzinfo=CST)
    ...
    for i in range(steps + 20):
        ...
        if i < 20:
            ...
            times.append(datetime.fromtimestamp(pre_h, tz=CST).strftime('%m-%d %H:%M'))
        elif i == 20:
            times.append(entry_dt.strftime('%m-%d %H:%M'))
        else:
            post_h = entry_dt.timestamp() + (i-20) * diff_hours * 3600 / steps
            times.append(datetime.fromtimestamp(post_h, tz=CST).strftime('%m-%d %H:%M'))
...
def main():
    ...
    # 只处理最近 2 个有数据的月份（历史月份的 paths 已存在且为真实K线）
    month_keys = sorted(months.keys())
    if len(month_keys) > 2:
        month_keys = month_keys[-2:]
    print(f'生成走势的月份: {month_keys}')

    # 拉取所有涉及币种的 15m K线（仅最近 2 个月涉及的币种）
    recent_trades = [t for t in trades if t['entry_time'][5:7] in month_keys]
    coins = sorted({t['coin'] for t in recent_trades})
